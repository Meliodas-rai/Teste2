...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5589981246187 :+55 89 981246187\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝙀𝙖𝙚 𝙢𝙚𝙪 𝙘𝙖𝙧𝙤 𝙜𝙖𝙛𝙖𝙣𝙝𝙤𝙩𝙤 @${num.split('@')[0]}\n𝙎𝙚𝙟𝙖 𝘽𝙚𝙢 𝙫𝙞𝙣𝙙𝙤 𝙖𝙤 𝙜𝙧𝙪𝙥𝙤 *${mdata.subject}*`
				...
				teks = `𝙁𝙡𝙬, 𝙫𝙖𝙞 𝙥𝙚𝙡𝙖 𝙨𝙤𝙢𝙗𝙧𝙖 𝙥𝙦 𝙗𝙤𝙨𝙩𝙖 𝙣𝙤 𝙨𝙤𝙡 𝙨𝙚𝙘𝙖 𝙠𝙨𝙠𝙨𝙠𝙨 @${num.split('@')[0]} 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo ✞︎Mᴇʟɪᴏᴅᴀs✞︎! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá como! Você não está registrado no banco de dados, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Toin|18`,
				}
			}
			...
			const ownerNumber = ["5589981246187@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5589981210976@whatsapp.net'
                        /******ApiKey Input******/
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('a tag alvo que o administrador deseja descartar')
              ...
              await client.sendMessage(from, options, text)
               break
					...
					try {
						...
						reply('nome de usuário tiktok válido')
					}
					break
				case 'snowwrite':
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Este é kpop sis...'})
                                        break
                                case 'fml2':
